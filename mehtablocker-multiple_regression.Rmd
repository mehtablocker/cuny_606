...
1.  Is this an observational study or an experiment? The original research
    ...
    __Observational, so the study should be re-phrased: "Whether there is a link between beauty and course evaluations."__

2.  Describe the distribution of `score`. Is the distribution skewed? What does 
    that tell you about how students rate courses? Is this what you expected to 
    see? Why, or why not?  
    __The distribution is left-skewed with a center near 4.3, indicating most scores are good but a few are really bad. This is unsurprising. We see a similar distribution in things like Yelp reviews.__
    ```{r ex_2}
    library(dplyr)
    library(ggplot2)
    
    evals %>% select(score) %>% summary()
    evals %>% ggplot(aes(score)) + geom_histogram(bins=15)
    ```

3.  Excluding `score`, select two other variables and describe their relationship 
    using an appropriate visualization (scatterplot, side-by-side boxplots, or 
    mosaic plot).  
    __As expected, there seems to be a negative correlation between age and average beauty score.__
    ```{r ex_3}
    evals %>% ggplot(aes(x=age, y=bty_avg)) + geom_point() + geom_jitter() + geom_smooth(method = "loess")
    ```
...
Is anything awry?  
__Yes, there appear to be fewer points than observations.__
...
4.  Replot the scatterplot, but this time use the function `jitter()` on the
    ...
    __Many of the points were directly on top of each other.__  
    ```{r ex_4}
...
5.  Let's see if the apparent trend in the plot is something more than
    ...
    summary(m_bty)
    evals %>% ggplot(aes(x=bty_avg, y=score)) + geom_point() + geom_jitter() + geom_smooth(method = "lm")
    ```  
    <br> __The equation is score = 3.88 + 0.067 x bty_avg, which means an increase of one bty_avg is associated with an increase of 0.067 score. The p-value is statistically significant, indicating that this coefficient for bty_avg would be highly unlikely if the true coefficient were 0. However, the R^2^ of `r summary(m_bty)$adj.r.squared` indicates an overall poor fit with this model.__
...
6.  Use residual plots to evaluate whether the conditions of least squares
    ...
    lm_df %>% 
      ...
      stat_qq_line()
    ```  
    <br> __The residual plot, histogram, and qqplot show indication of non-linearity, which could partly account for the poor model fit.__
...
7.  P-values and parameter estimates should only be trusted if the
    ...
    lm_df %>% 
      ...
      stat_qq_line()
    ```  
    <br> __The residual plots still show some non-linearity.__
    

8.  Is `bty_avg` still a significant predictor of `score`? Has the addition
    of `gender` to the model changed the parameter estimate for `bty_avg`?  
    __It is still significant and the coefficient has changed a little bit but not very much.__  
...
9.  What is the equation of the line corresponding to males? (*Hint:* For
    ...
    \[
      \begin{aligned}
    \widehat{score} &= \hat{\beta}_0 + \hat{\beta}_1 \times bty\_avg + \hat{\beta}_2 \end{aligned}
    \]  
    __Males have a higher score, beauty being equal.__
...
10. Create a new model called `m_bty_rank` with `gender` removed and `rank` 
    ...
    summary(m_bty_rank)
    ```  
    <br> __R correctly drops one of the three because it is a linear combination of the other two.__
...
12. Check your suspicions from the previous exercise. Include the model output
    in your response.  
    __My suspicion was incorrect. Number of credits has a very low p-value.__

13. Interpret the coefficient associated with the ethnicity variable.  <br> __Non minorities get a predicted score of 0.12 higher than minorities.__
...
14. Drop the variable with the highest p-value and re-fit the model. Did the
    ...
    ```{r ex_14}
    m_full_mns_one <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
    summary(m_full_mns_one)
    ```  
    <br> __After dropping cls_profs the other variables did not change much. This indicates that it had low collinearity, i.e., it was fairly independent of the other variables.__
...
15. Using backward-selection and p-value as the selection criterion,
    ...
    summary(m_best)
    ```  
    <br> __score = 3.77 + 0.17 x ethnicity + 0.21 x gender - 0.21 x language - 0.01 x age + 0.005 x cls_perc_eval + 0.51 x cls_credits + 0.05 x bty_avg - 0.19 x pic_color__

16. Verify that the conditions for this model are reasonable using diagnostic 
    plots.
    ```{r ex_16}
    lm_df <- augment(m_best)
    ...
    lm_df %>% 
      ...
      stat_qq_line()
    ```  
    <br> __The residual plots still show a bit of non-linearity. And the R^2^ indicates a fairly poor fit.__
...
17. The original paper describes how these data were gathered by taking a
    ...
    __Yes, it could violate the assumption of data sample independence.__

18. Based on your final model, describe the characteristics of a professor and 
    course at University of Texas at Austin that would be associated with a high
    evaluation score.  
    __A good looking, young, non minority, male who speaks English natively, teaches a one credit class with high participation, and has a black-and-white picture.__

19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?  
    __Though maybe in the vague direction, I would not be too vehement in my conclusions given the relatively poor model fit, the questionable residual plots, the potential lack of sampling independence, and the lack of consideration of prior coefficient values.__
