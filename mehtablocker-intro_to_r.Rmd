...
```{r plot-proportion-vs-year, eval=TRUE}
plot(arbuthnot$year, arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls), type = "l", xlab="Year", ylab="Proportion of Boys")
```
...
Not even close. The present day counts are much higher.
...
```{r oyo_3, eval=TRUE}
plot(present$year, present$boys/(present$boys+present$girls), type="l", xlab="Year", ylab="Proportion of Boys")
```

The proportion of boys is above 0.5, but lower than in the Arbuthnot data. Also there appears to be a downward trend.
...
```{r oyo_4, eval=TRUE}
present %>% mutate(total=boys+girls) %>% arrange(desc(total)) %>% slice(1)
```
