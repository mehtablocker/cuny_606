...
**Streak length of 1 means one hit followed by one miss. Streak length of 0 means zero hits followed by one miss.**
...
**The distribution is positive and right skewed, and has a shape resembling a geometric distribution (i.e., discrete version of exponential.)**

```{r ex_2}
### typical streak length or "mode" of distribution:
as.numeric(names(tail(sort(table(kobe_streak)), 1)))
...
# The distribution is positive and right skewed, like Kobe's actual, but the right tail is longer in the sim.

# The typical "mode" streak is the same
as.numeric(names(tail(sort(table(sim_streak)), 1)))

# But the longest streak is higher
max(sim_streak)
...
**Due to sampling error it will not be exactly the same, but it will be very similar given that the underlying distribution is the same. And given that the sample size is 133, that's probably large enough such that some of the robust summary metrics (e.g., mode) will not likely change.**
...
**The distributions look very similar, as can be seen in an overlapping density plot:**

```{r oyo_3}
plot(density(sim_streak), main="Kobe vs Sim", xlab="Streak Lengths", col="red")
lines(density(kobe_streak))
legend("topright", legend=c("Kobe", "Sim"), lty=c(1,1), col=c("black", "red"))
```

**If anything, Kobe has *fewer* long streaks than would be expected by chance alone.**

**This (small) study reiterates what most studies on this topic tend to find: that there isn't convincing evidence of a hot hand effect.**

**FYI, there was a fairly recent study (see Zweibel) that claimed to find new evidence for the hot hand effect. His assumptions were fully dissected on a blog by Tom Tango, Mitchell Lichtman, and other well known baseball statisticians. What they found was a conflation between "hot hand" and "recency."**

**In other words, most performance projection models account for the fact that talent (i.e., the underlying parameter - Kobe's true shooting percentage in this example) is not static but constantly changing. They often model that by weighting recent results more heavily than older results (in the same way one might weight stock returns exponentially over a time series to get a more accurate projection, etc.)**

**Once that weighting is introduced, any hot hand effect tends to get mitigated or disappear completely. If no weighting is done, one can get results that sometimes look like a hot hand effect. So to some extent, belief in a hot hand comes down to semantics / philosophy of how it should be defined in the first place.**
