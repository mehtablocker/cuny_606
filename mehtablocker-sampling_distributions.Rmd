...
__It has a bit of a right skew, as can be verified by a normal plot:__
```{r qqnorm_area}
qqnorm(area)
qqline(area)
```
...
2.  Describe the distribution of this sample. How does it compare to the 
    ...
    __The distribution is similar to that of the population.__
...
__The mean is similar but not exact. The sample size of 1000 will be a more accurate estimate.__
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    __There are 5000 elements. The distribution is gaussian with mean very close to the true mean. The spread of the distribution would not change much with 50000 elements.__
...
__There are 100 elements. Each element is a mean of sample size 50.__
...
__The center does not change but the spread gets smaller as n increases.__
...
-   Since you have access to the population, simulate the sampling 
    ...
    ```{r oyo_2}
    sample_means50 <- replicate(5000, mean(sample(price, 50)))
    hist(sample_means50, breaks = 25)
    
    # The shape is gaussian. I'd expect the mean to be:
    mean(sample_means50)
    # The true mean is very close at:
    mean(price)
    ```
...
-   Change your sample size from 50 to 150, then compute the sampling 
    ...
    ```{r oyo_3}
    sample_means150 <- replicate(5000, mean(sample(price, 50)))
    hist(sample_means150)
    
    # This distribution is also normally shaped, but the spread is narrower.
    # The mean should not be much different than what we calculated previously.
...
-   Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    __The latter distribution has a smaller spread and is the more preferable estimate since it is a lower standard error for our estimation.__
