...
# Can see from the plots that the distribution has a bit of right skew. This is confirmed by the mean being higher than the median. I would probably use the median to describe the "typical" value.
```

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

__It will not be identical, due to sampling error, but it will be similar due to the sample size of 60.__
...
__The sample size must be large enough. How large depends in part on how non-gaussian the underlying distribution is.__
...
__It means that if you took repeated intervals like this, 95% of them would include the true mean. It does NOT imply a probability about the parameter.__
...
__It does but a neighbor's interval might not if he/she got "unlucky" with the sample they originally took.__
...
__95% of them will. That is the definition of a CI.__
...
-   Using the following function (which was downloaded with the data set), 
    ...
    __49/50 (98 percent), which is close but not exactly equal to the confidence level due to sampling error.__

-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
    
    __Critical value for 90 percent CI is 1.645__
