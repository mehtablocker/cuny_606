...
**There are 20000 cases (rows) and 9 variables (columns.)  
genhlth: categorical (nominal)  
exerany: categorical (nominal) that has been converted to 0/1 numeric  
hlthplan: categorical (nominal) that has been converted to 0/1 numeric  
smoke100: categorical (nominal) that has been converted to 0/1 numeric  
height: numerical (discrete)  
weight: numerical (discrete)  
wtdesire: numerical (discrete)  
age: numerical (discrete)  
gender: categorical (nominal)  
NOTE that the four numerical variables could be modeled as continuous, but here they are discrete.**  
...
```{r ex_2, warning=F, message=F}
library(dplyr)
library(DATA606)

### Summary of height and age
cdc %>% select(height, age) %>% summary()

### Relative freq for gender and exerany
summary(cdc$gender)/length(cdc$gender)
mean(cdc$exerany==1)

### Number of males
cdc %>% filter(gender=="m") %>% nrow()
...
**Percentage of males who smoked at least 100 cigarettes was higher than percentage of females.**
...
knitr::kable(under23_and_smoke %>% head())
```
...
**The boxplot shows the distribution of bmi by each category of genhlth.**

```{r ex_5}
boxplot(bmi ~ cdc$gender, main="BMI by Gender")
```

**The above boxplot shows that, as expected, the distribution of BMI is centered higher for males than females.**
...
### As expected, there appears to be a positive correlation (and also heteroscedasticity) between weight and desired weight.
...
### The data is numerical. 0 means the person is at his or her desired weight. A positive value indicates the person wants to gain weight. A negative value indicates the person wants to lose weight.
...
```{r oyo_4}
summary(wdiff)
hist(wdiff, breaks=100, xlim=c(-150, 150))

### The median is -10, indicating that the typical person wants to lose about 10 pounds. Some people want to lose a lot of weight, as indicated by the left skew seen in the histogram, as well as the fact that the mean is almost five pounds lower than the median.
```
...
```{r oyo_5}
cdc$wdiff <- wdiff
### wdiff summary for men
cdc %>% filter(gender=="m") %>% select(wdiff) %>% summary()
### wdiff summary for women
cdc %>% filter(gender=="f") %>% select(wdiff) %>% summary()
boxplot(cdc$wdiff ~ cdc$gender, ylim=c(-250, 150))

### Women, on average, wanted to lose more weight than men.
```
...
mean(cdc$weight)
sd(cdc$weight)
cdc <- cdc %>% mutate(z_weight=(weight-mean(weight))/sd(weight))
mean(cdc$z_weight<=1 & cdc$z_weight>=(-1))
```
