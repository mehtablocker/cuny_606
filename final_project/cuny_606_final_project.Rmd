...
Now that our data are transformed and tidy, we explore a few plots of the variables.  

A histogram of Weekly Sales indicates outlier values, so we filter out everything above 150 thousand and try again.

```{r weekly_sales_hist}
df %>% ggplot(aes(Weekly_Sales)) + geom_histogram(bins=50)
df <- df %>% filter(Weekly_Sales<150000)
df %>% ggplot(aes(Weekly_Sales)) + geom_histogram(bins=50)
```
...
Now we fit a multiple linear regression model to formally examine the relationship between Weekly Sales and Temperature + Fuel Prices.
...
The Adjusted R-squared is terribly low. We also examine the residual plots.
