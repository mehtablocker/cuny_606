...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    assumption?
__Independence and sample size. The total sample is less than 10% of the world's population, and there is a sample size of np>10 in each bucket. I'd be a bit curious about selection bias. They mention they used a combination of face-to-face interviews, telephone calls, and online surveys. But if a certain large percentage of the population is too poor to be reached via typical methods, and there is a correlation between wealth and religiosity, there could be bias in the samples. But I will give the researchers the benefit of the doubt.__
...
__Each row of Table 6 is the aggregation by country. Each row of our data frame is an individual respondent.__
...
__Independence and sample size. Each expected bucket count (np) should be greater than 10. The conditions for inference here are reasonable.__
...
__Here the margin of error is 4.99 - 3.64 = 1.35 percent__
...
inference(france12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
# The conditions are met. The margin of error is:
0.2873 - 0.2657
...
__The relationship is concave, with a global maximum at 0.5__
...
9.  Describe the sampling distribution of sample proportions at $n = 1040$ and 
    ...
    *Hint:* Remember that R has functions such as `mean` to calculate summary
    statistics.
__The distribution is gaussian shaped with a center very near the value of p and a spread equal to the standard error.__
...
__While there is a bit of right skew for Ecuador, it doesn't appear to be too drastic. I'd say it's ok to proceed.__
...
-   Answer the following two questions using the `inference` function. As 
    ...
    *Hint:* Create a new data set for respondents from Spain. Form
    ...
    ```
    __No, the confidence intervals overlap significantly.__
    ...
    **b.** Is there convincing evidence that the United States has seen a
    ...
    ```
    __Yes, the confidence intervals are far apart.__
...
-   If in fact there has been no change in the atheism index in the countries 
    ...
    *Hint:* Look in the textbook index under Type 1 error.
    __By definition, 5 percent or about 2 countries.__
...
-   Suppose you're hired by the local government to estimate the proportion of 
    ...
    *Hint:* Refer to your plot of the relationship between $p$ and margin of 
    ...
    __Since we know margins of error are highest when p=0.5, we can use that after rewriting the ME = z_score * SE formula to solve for n:   n = pq / (ME/z_score)^2 which in this case is 9604.__
