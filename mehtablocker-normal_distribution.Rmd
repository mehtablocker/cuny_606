...
hist(mdims$hgt)
hist(fdims$hgt)
plot(density(mdims$hgt, adjust=1.5), main="Male vs Female Heights", xlab="Height in cm")
lines(density(fdims$hgt, adjust = 1.5), col="red")
legend("topright", legend=c("male", "female"), lty=c(1,1), col=c("black", "red"))
mean(mdims$hgt); mean(fdims$hgt)
sd(mdims$hgt); sd(fdims$hgt)
```

**Men are clearly taller, and the spread is a bit wider as well.  (PS - Thanks for the tip on the "adjust" parameter within the density() function!)**
...
**Not all of the points are exactly on the line, but overall it looks fairly similar to the real data.**
...
**No, there seems to be a pretty significant right skew to the distribution.**
...
**What percent of females have heights below 160? What percent of females have weights below 60?**

```{r ex_6}
#height, theoretical
pnorm(160, mean(fdims$hgt), sd(fdims$hgt))

#height, empirical
mean(fdims$hgt<160)

#weight, theoretical
pnorm(60, mean(fdims$wgt), sd(fdims$wgt))
...
**Theoretical is closer to empirical for heights than for weights, as expected, because of the shapes of the distributions.**
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    **Because the empirical distribution has fewer data points in the center of the assumed distribution.**
